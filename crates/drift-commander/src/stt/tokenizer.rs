...
impl ParakeetTokenizer {
    pub fn from_dir(model_dir: impl AsRef<Path>) -> anyhow::Result<Self> {
        ...
        if !tokens_path.exists() {
            anyhow::bail!("tokens.txt not found: {}", tokens_path.display());
        }
        ...
        for line in contents.lines() {
            ...
            // sherpa-onnx format: "token id" (space-separated, token may contain spaces)
            let last_space = line.rfind(' ').ok_or_else(|| {
                anyhow::anyhow!("invalid tokens.txt line (no space): {}", line)
            })?;
